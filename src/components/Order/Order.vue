...
<!--        订单列表数据-->
        <el-table :data="orderList" border stripe>
          ...
          <el-table-column label="是否付款" prop="pay_status" width="200px">
...
<!--      时间线-->
      <el-timeline>
        <el-timeline-item
            v-for="(activity, index) in progressKuaidi"
            :key="index"
            :icon="activity.icon"
            :type="activity.type"
...
export default {
  ...
  methods: {
    ...
    async showProgressBox(){
      ...
      if (res.meta.status!==200 ){return this.$message.error("获取快递物流失败！")}
      console.log(res.data)
      this.progressKuaidi = res.data
    },
